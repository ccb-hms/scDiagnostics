...
common_genes <- intersect(rownames(qc_data), rownames(reference_data))
qc_data <- qc_data[intersect(rownames(qc_data), rownames(reference_data)),]
reference_data <- reference_data[intersect(rownames(qc_data), rownames(reference_data)),]
...
SummarizedExperiment::colData(qc_data) <- SummarizedExperiment::colData(qc_data)[, c("total",
                                                                                     "SingleR_annotation", 
                                                                                     "annotation_scores")]

# Remove counts assays
SummarizedExperiment::assays(qc_data) <- 
    SummarizedExperiment::assays(qc_data)[-which(names(SummarizedExperiment::assays(qc_data)) == "counts")]

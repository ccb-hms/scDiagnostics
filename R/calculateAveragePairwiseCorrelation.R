...
#'
#' @details
#' This function operates on \code{\link[SingleCellExperiment:SingleCellExperiment-class]{SingleCellExperiment}} objects,
...
calculateAveragePairwiseCorrelation <- function(query_data,
    ...
    .computeCorrelation <- function(type1, type2) {
        if (!is.null(pc_subset)) {
            ...
            query_subset <- query_data[, which(
                query_data[[query_cell_type_col]] == type1
            ), drop = FALSE]
            ref_subset <- reference_data[, which(
                reference_data[[ref_cell_type_col]] == type2
            ), drop = FALSE]
    ...
    class(cor_matrix_avg) <- c(
        class(cor_matrix_avg),

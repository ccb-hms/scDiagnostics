...
#'
#' @description This function takes a matrix of category scores (cell
#'     type by cells) and calculates the entropy of the category
#'     probabilities for each cell. This gives a sense of how
#'     confident the cell type assignments are.  High entropy = lots
#'     of plausible category assignments = low confidence. Low entropy
#'     = only one or two plausible categories = high confidence. This
#'     is confidence in the vernacular sense, not in the
#'     "confidence interval" statistical sense. Also note that the
#'     entropy tells you nothing about whether or not the assignments
#'     are correct -- see the other functionality in the package for
#'     that. This functionality can be used for assessing how
#'     comparatively confident different sets of assignments are
#'     (given that the number of categories is the same).
#'
#' @details The function checks if X is already on the probability
#'     scale.  Otherwise, it applies softmax columnwise.
#'
#'     You can think about entropies on a scale from 0 to a maximum
#'     that depends on the number of categories. This is the function
#'     for entropy (minus input checking): \code{entropy(p) =

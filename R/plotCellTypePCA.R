...
plotCellTypePCA <- function(query_data,
    ...
    data_pairs_list <- lapply(seq_len(nrow(pairs)), function(i) {
        ...
        data_frame <- data.frame(
            pca_output[, c(pairs[["x"]][i], pairs[["y"]][i])],
    ...
    data_pairs[["cell_type_dataset"]] <- factor(
        data_pairs[["cell_type_dataset"]], levels = order_combinations)
    cell_type_colors <- generateColors(order_combinations, paired = TRUE)

    # Extract selected cell types
    data_pairs <- data_pairs[pca_output[["cell_type"]] %in% cell_types, ]

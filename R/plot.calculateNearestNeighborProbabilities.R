...
plot.calculateNearestNeighborProbabilities <- function(x, 
                                                       cell_types = NULL, 
                                                       ...) {
    ...
    densities <- do.call(rbind, lapply(seq_len(length(x)), function(i) {
        x_range <- seq(0.5 - 3 * sqrt(0.25 / x[[i]][["n_neighbor"]] / 
                                          x[[i]][["n_query"]]), 
                       0.5 + 3 * sqrt(0.25 / x[[i]][["n_neighbor"]] / 
                                          x[[i]][["n_query"]]), 
                       length.out = 1000)
        data.frame(x = x_range, y = dnorm(
            x_range, mean = 0.5, 
            sd = sqrt(0.25 / 
            x[[i]][["n_neighbor"]] / x[[i]][["n_query"]])), 
            cell_type = names(x)[i],
            query_prob = x[[i]][["query_prob"]],
            lb = qnorm(
                0.025, 
                mean = 0.5, 
                sd = sqrt(0.25 / x[[i]][["n_neighbor"]] / x[[i]][["n_query"]])),
            ub = qnorm(
                0.975, 
                mean = 0.5, 
                sd = sqrt(0.25 / x[[i]][["n_neighbor"]] / x[[i]][["n_query"]])))
    ...
    cell_type_colors <- generateColors(sort(unique(densities[["cell_type"]])), 
                                       paired = FALSE)
    
    # Plot the density functions with facets and quantile lines
    density_plot <- ggplot2::ggplot(
        densities, ggplot2::aes(
            x = .data[["x"]], y = .data[["y"]], 
            fill = .data[["cell_type"]])) +
        ...
        ggplot2::scale_fill_manual(values = cell_type_colors, 
                                   name = "Cell Type") + 
        ggplot2::geom_vline(
            data = densities, ggplot2::aes(xintercept = .data[["query_prob"]]), 
            linetype = "dashed", color = "black") +
        ggplot2::geom_vline(
            data = densities, ggplot2::aes(xintercept = .data[["lb"]]), 
            linetype = "solid", color = "black") +
        ggplot2::geom_vline(
            data = densities, ggplot2::aes(xintercept = .data[["ub"]]), 
            linetype = "solid", color = "black") +
        ggplot2::labs(
            title = "Approximate Density Functions Under Null Hypothesis (Equal Reference and Query Distributions)", 

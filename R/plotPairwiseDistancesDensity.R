...
#'
#' @description This function calculates pairwise distances or correlations
#' between query and reference cells of a specified cell type and visualizes the
...
#' @param cell_type The cell type for which distances or correlations are
#' calculated.
#' @param pc_subset A numeric vector specifying which principal components to
#' use in the analysis. Default is 1:5. If set to \code{NULL}, the assay data is
#' used directly for computations without dimensionality reduction.
#' @param distance_metric The distance metric to use for calculating pairwise
#' distances, such as euclidean, manhattan, etc. Set to "correlation" to
#' calculate correlation coefficients.
#' @param correlation_method The correlation method to use when
#' \code{distance_metric} is "correlation". Possible values are "pearson" and
#' "spearman".
#' @param bandwidth Numeric value controlling the smoothness of the density
#' estimate; smaller values create more detailed curves. Default is 0.25.
...
#' Default is 5000.
#'
#' @return A ggplot2 object showing ridgeline plots of calculated distances or
#' correlations.
#'
#' @export
#'
...
plotPairwiseDistancesDensity <- function(query_data,
    ...
    query_query_name <- paste(
        "Query", cell_type,
        "vs", "Query", cell_type
    )
    query_ref_name <- paste(
        "Query", cell_type,
        "vs", "Ref", cell_type
    )
    dist_df <- data.frame(

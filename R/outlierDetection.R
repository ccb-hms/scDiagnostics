...
#'     matrix
#'
#' @param group.var if specified, run the outlier scoring by group
#'     (e.g. cell cluster) instead of across all cells globally.
#'
...
calculateOutlierScore <- function(sce,
    ...
    if (!is.null(group.var)) {
        if (is.null(colnames(sce))) {
            stop("This function requires the input ",
                 "SCE to have unique column names ",
                 "when calculating group-wise outlier scores.")
        }

        ## There's probably a better way to split an sce by a
        ## factor...  Anyway, manually passing it through factor()
        ## like this way retains NAs.
        cluster_list <- lapply(
            split(
                colData(sce),
        ...
        ## TODO: assess if it would be worth making the threshold
        ## adaptive. Different clusters can have different
        ## distributions of outlier scores.
        ...
        ## If we were using an ID column instead of column names of
        ## the input we could do a join.
        score_df <- score_df[rownames(colData(sce)), ]
        colData(sce) <- cbind(colData(sce), score_df)

        S4Vectors::metadata(sce)$cluster_isotree_model <-
                                   lapply(
                                       score_list,
                                       S4Vectors::metadata
                                   )

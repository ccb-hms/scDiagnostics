...
plot.calculateDiscriminantSpaceObject <- function(
    ...
    if(is.null(cell_types)){
        ...
        extracted_combinations <- sapply(names(x),
                                         function(t)
                                             return(sum(sapply(cell_types, grepl, t)) == 2))
        if(all(!extracted_combinations)){
            stop("There are no combinations of the \'cell_types\' to plot.")
        }
        x <- x[extracted_combinations]
    }
    ...
    if(plot_type == "scatterplot"){
        ...
        plot_data <- rbind(data.frame(x[[1]][["ref_proj"]],
                                      data_type = rep("Reference", nrow(x[[1]][["ref_proj"]]))),
                           data.frame(x[[1]][["query_proj"]],
                                      data_type = rep("Query", nrow(x[[1]][["query_proj"]]))))
        ...
        if(length(names(x)) > 1){
            for(comb_id in 2:length(names(x))){
                # Formatting of plot data
                plot_data <- rbind(data.frame(x[[comb_id]][["ref_proj"]],
                                              data_type = rep("Reference",
                                                              nrow(x[[comb_id]][["ref_proj"]]))),
                                   data.frame(x[[comb_id]][["query_proj"]],
                                              data_type = rep("Query",
                                                              nrow(x[[comb_id]][["query_proj"]]))))
                # Create a new variable representing the combination of cell type and dataset
                plot_data[["cell_type_dataset"]] <- paste(
                    plot_data[["data_type"]], plot_data[["cell_type"]], sep = " ")
                plot_data[["cell_type_combination"]] <- paste0(
                    sort(unlist(strsplit(names(x)[comb_id], "-"))), collapse = "-")
                full_data <- rbind(
                    full_data[, c("DV1", "DV2", "cell_type_dataset", "cell_type_combination")],
        ...
        full_data[["cell_type_dataset"]] <- factor(full_data[["cell_type_dataset"]],
                                                   levels = order_combinations)

        # Generate scatter plot
        scatter_plot <- ggplot2::ggplot(full_data,
                                        ggplot2::aes(x = .data[["DV1"]],
                                                     y = .data[["DV2"]],
        ...
        # Formatting of plot data
        plot_data <- rbind(data.frame(x[[names(x)]][["ref_proj"]],
                                      data_type = rep("Reference",
                                                      nrow(x[[names(x)]][["ref_proj"]]))),
                           data.frame(x[[names(x)]][["query_proj"]],
                                      data_type = rep("Query",
                                                      nrow(x[[names(x)]][["query_proj"]]))))
        ...
        # Create a new variable representing the combination of cell type and dataset
        data_long[["cell_type_dataset"]] <- paste(data_long[["data_type"]],
                                                  data_long[["cell_type"]], sep = " ")
        ...
        # Reorder the levels of cell type and dataset factor
        data_long[["cell_type_dataset"]] <- factor(data_long[["cell_type_dataset"]],
                                                   levels = order_combinations)
        ...
        box_plot <- ggplot2::ggplot(data_long,

...
plot.calculateDiscriminantSpaceObject <- function(
            ...
            for (ct in cell_types) {
                ...
                if (nrow(ct_data) > 0) {
                    ...
                    n_cells_ct <- min(
                        nrow(ct_data),
                        max(1, round(
                            max_cells_query * nrow(ct_data) / n_query_total
                        ))
                    )
                    if (nrow(ct_data) > n_cells_ct) {
                        sampled_indices <- sample(nrow(ct_data), n_cells_ct)
    ...
    .smoothContourFunc <- function(data, mapping, ...) {
        ...
        # Cell type/dataset column and its levels do not change across the
        # loop below, so extract them once rather than every iteration

...
#'
#' This function generates density plots to visualize the distribution
#' of gene expression values for a specific gene across the overall
#' dataset and within a specified cell type.
#'
#' @details This function generates density plots to compare the
#'     distribution of a specific marker gene between reference and
#'     query datasets. The aim is to inspect the alignment of gene
#'     expression levels as a surrogate for dataset
#'     similarity. Similar distributions suggest a good alignment,
#'     while differences may indicate discrepancies or
#'     incompatibilities between the datasets.
#' 
#' @param reference_data A \code{\linkS4class{SingleCellExperiment}}
#'     object containing numeric expression matrix for the reference
#'     cells.
#'
#' @param query_data A \code{\linkS4class{SingleCellExperiment}}
#'     object containing numeric expression matrix for the query
#'     cells.
#'
#' @param reference_cell_labels character. The column name in the
#'     \code{colData} of \code{reference_data} that identifies the
#'     cell types.
#'
#' @param query_cell_labels character. The column name in the
#'     \code{colData} of \code{query_data} that identifies the cell
#'     types.
#'
#' @param gene_name character. A string representing the gene name for
#'     which the distribution is to be visualized.
#'
#' @param label character. A vector of cell type labels to plot (e.g.,
#'     c("T-cell", "B-cell")).
#'
#' @return A gtable object containing two arranged density plots as
#'     grobs.  The first plot shows the overall gene expression
#'     distribution, and the second plot displays the cell
#'     type-specific expression distribution.
#'         
#' @examples
#' library(scater)
#' library(scran)
...
#' pred <- SingleR(query_data,
#'     ref_data,
#'     labels = ref_data$reclustered.broad
#' )
#'
#' # Add labels to query object
#' colData(query_data)$labels <- pred$labels
#'
#' # Note: Users can use SingleR or any other method to obtain the 
...
#'                      label = "B_and_plasma")
#'
#' @importFrom ggplot2 ggplot aes geom_density theme_minimal
             ...
             label) 
{
    ...
    dataset_labels <- rep(c("Reference", "Query"),
                          times = c(length(reference_gene_expression), 
    ...
    overall_plot <-
        ggplot(data,
               aes(x = .data$GeneExpression, fill = .data$Dataset)) +
    ...
    reference_gene_expression_cell_type <-
        assay(reference_data, "logcounts")[gene_name, index1]
    query_gene_expression_cell_type <-
        assay(query_data, "logcounts")[gene_name, index2]
    
    ## Combine the gene expression values and dataset labels for cell
    ## type-specific
    ...
    ## Create a stacked density plot using ggplot2 for cell
    ## type-specific dataset
    cell_type_specific_plot <-
        ggplot(cell_type_specific_data, 
               aes(x = .data$GeneExpression, fill = .data$Dataset)) +
        geom_density(alpha = 0.5) +
    ...
    grid.arrange(overall_plot, cell_type_specific_plot, ncol = 2)
}

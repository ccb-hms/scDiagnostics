...
#'
#' @description This function projects a query \code{SingleCellExperiment}
#' object onto the SIR (supervised independent component) space of a reference
#' \code{SingleCellExperiment} object. The SVD of the reference data is computed
#' on conditional means per cell type, and the query data is projected based on
#' these reference components.
#'
#' @details The genes used for the projection (SVD) must be present in both the
#' reference and query datasets. The function first computes conditional means
#' for each cell type in the reference data, then performs SVD on these
#' conditional means to obtain the rotation matrix used for projecting both the
#' reference and query datasets. The query data is centered and scaled based on
#' the reference data.
#'
...
#' @param query_cell_type_col A character string specifying the column in the
#' \code{colData} of \code{query_data} that identifies the cell types.
#' @param ref_cell_type_col A character string specifying the column in the
#' \code{colData} of \code{reference_data} that identifies the cell types.
#' @param cell_types A character vector of cell types for which to compute
#' conditional means in the reference data.
#' @param multiple_cond_means A logical value indicating whether to compute
#' multiple conditional means per cell type (through PCA and clustering).
#' Defaults to \code{TRUE}.
#' @param cumulative_variance_threshold A numeric value between 0 and 1
#' specifying the variance threshold for PCA when computing multiple conditional
#' means. Defaults to \code{0.7}.
#' @param n_neighbor An integer specifying the number of nearest neighbors for
#' clustering when computing multiple conditional means. Defaults to \code{1}.
#' @param assay_name A character string specifying the assay name on which to
#' perform computations. Defaults to \code{"logcounts"}.
...
#' \item{cond_means}{A matrix of the conditional means computed for the
#' reference data.}
#' \item{rotation_mat}{The rotation matrix obtained from the SVD of the
#' conditional means.}
#' \item{sir_projections}{A \code{data.frame} containing the SIR projections for
#' both the reference and query datasets.}
#' \item{percent_var}{The percentage of variance explained by each component of
#' the SIR projection.}
#'
#' @export
#'
...
#'
#' @description This function computes conditional means for each cell type in
#' the reference data. It can compute either a single conditional mean per cell
...
#' @param ref_cell_type_col A character string specifying the column in
#' \code{colData(reference_data)} that contains the cell type labels.
#' @param cell_types A character vector of cell types for which to compute
#' conditional means.
#' @param multiple_cond_means A logical value indicating whether to compute
#' multiple conditional means per cell type. Defaults to \code{FALSE}.
#' @param assay_name A character string specifying the name of the assay to use
#' for the computation. Defaults to \code{"logcounts"}.
#' @param cumulative_variance_threshold A numeric value between 0 and 1
#' specifying the variance threshold for PCA when computing multiple conditional
#' means. Defaults to \code{0.7}.
#' @param n_neighbor An integer specifying the number of nearest neighbors for
#' clustering when computing multiple conditional means. Defaults to \code{1}.
#'
#' @return A numeric matrix with the conditional means for each cell type. If
#' \code{multiple_cond_means = TRUE}, the matrix will contain multiple rows for
#' each cell type, representing the different conditional means computed via
#' clustering.
#'
#' @keywords internal
#'

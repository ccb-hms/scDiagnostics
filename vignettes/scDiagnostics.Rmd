...
```{r QC-plot-QC-vs-Annotation-1, message=FALSE, warning=FALSE, eval=TRUE}
...
```{r QC-plot-QC-vs-Annotation-2, warning=FALSE, message=FALSE, eval=TRUE}
# load query dataset
...
```{r hist-QC-vs-Annotation, warning=FALSE, message=FALSE, eval=TRUE}
# Generate histogram
...
```{r plot-Gene-Set-Scores, warning=FALSE, message=FALSE}
...
datasets. Factors such as batch effects, differences in sample
...
```{r visualize-Cell-Type-MDS, warning=FALSE, message=FALSE}
...
```{r visualize-Cell-Type-PCA, warning=FALSE, message=FALSE}
...
The nearestNeighborDiagnostics function is designed to compute the
probabilities that each sample in a query dataset belongs to either the
reference or the query dataset for each cell type using nearest neighbor
analysis. This function is particularly useful for evaluating the
consistency of cell type classification across datasets.

This function projects the query data onto the PCA space of the
reference data, balances the sample sizes between the reference and
...
nn_output <- nearestNeighborDiagnostics(query_data = query_data_subset, 
                                        reference_data = ref_data_subset,
                                        query_cell_type_col = "labels", 
                                        ref_cell_type_col = "reclustered.broad")
...
## Compute Sample Distances Between Reference and Query Data

The calculateSampleDistances function is designed to analyze the spatial
...
query samples to reference samples. Using PCA for dimensionality
reduction and Euclidean distance for the actual distance calculations,
this function helps in understanding how closely the query samples
...
datasets. For instance, large distances from query samples to reference
samples could indicate that the query data contains different or novel
cell types not well-represented in the reference dataset. Conversely,
small distances suggest that the query samples closely match the
...
```{r calculate-Sample-Distances, warning=FALSE, message=FALSE, fig.width=8}
...
```{r calculate-Sample-Distances-CD8, warning=FALSE, message=FALSE, fig.width=8}
# Plot the densities of the distances for the anomalies
...
## Compute Bhattacharyya Coefficients and Hellinger Distances on Sample Distances
...
query samples and reference data.

```{r calculate-Sample-Distances-Similarity, warning=FALSE, message=FALSE, fig.width=8}
...
```{r calculate-Sample-Distances-Similarity-Hellinger, warning=FALSE, message=FALSE, fig.width=8}
overlap_measures$hellinger_dist
```

When employing the calculateSampleDistancesSimilarity function, the
...
## Calculate Sample Similarity to PC Loadings

This function aims to compute the cosine similarity between samples
...
examination of how anomalous samples align with principal components,
...
```{r calculate-Sample-Similarity-PCA, warning=FALSE, message=FALSE, fig.width=8}
